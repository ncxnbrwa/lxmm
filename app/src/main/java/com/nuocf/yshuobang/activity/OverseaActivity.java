...
/**
 * @author xiong
 * @ClassName: OverseaActivity
 * @Description: todo(海外医疗界面)
 * @date 2016/8/26
 */

@ContentView(R.layout.activity_oversea)
public class OverseaActivity extends BaseActivity {
    @ViewInject(R.id.progressBar1)
    private ProgressBar progressBar1;
    @ViewInject(R.id.wv_oversea)
    private WebView wv;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        wv.loadUrl(APIConstants.OVERSEA);
        ...
        switch (v.getId()) {
            ...
            case R.id.register_now:
                if (!TextUtils.isEmpty(ELS.getInstance(getBaseActivity()).getStringData(ELS.SESSION_KEY))) {
                    Bundle b = new Bundle();
                    //海外
                    b.putInt(AppointmentActivity.INTENT_BUNDLE_ORDER, Config.ORDER_HAIWAI);
